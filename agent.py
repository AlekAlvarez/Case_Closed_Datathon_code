...
checkpoint = torch.load('model_state.txt')
...
def train_model():
    ...
    if game_count%10==0:
        ...
        torch.save(state, "./model_state.txt")
...
def send_move():
    ...
    if turn<4:
        top_values, top_indices = torch.topk(log_probs_all.squeeze(0), k=4)
    
        # b) Select a random index from 0 to K-1
        random_idx = torch.randint(low=0, high=4, size=(1,)).item()
    
        # c) Get the actual move index tensor
        move= top_indices[random_idx].unsqueeze(0)
    
        # d) Calculate Log Probability for the chosen move.
        # We record the probability of the chosen move *under the policy's distribution*.
        # This correctly trains the policy to favor the chosen move even if it wasn't argmax.
        log_prob_of_chosen_action = dist.log_prob(move).unsqueeze(0)
    turn+=1
    match move:
        case 0:
            move='UP'
