...
checkpoint = torch.load('aggressive_model_state.txt')
...
def train_model():
    ...
    if game_count%20==0:
        with open("agg_data.txt", "a") as file:
            file.write(str(sum(reward)/len(reward))+'\n')
        torch.save(state, "./aggressive_model_state.txt")
...
def send_move():
    ...
    # 2. Select the final move (index) based on the current turn
    if turn < 4:
        # PHASE 1: PURE UNIFORM RANDOM EXPLORATION
        
        # Select a random integer between 0 and 7
        final_move_tensor = torch.randint(low=0, high=8, size=(1,)).squeeze(0)
        
        # Log probability must be that of a uniform distribution (log(1/8))
        log_prob_of_chosen_action = torch.log(torch.tensor(1.0 /8)).unsqueeze(0)
        
    else:
        # PHASE 2: GREEDY EXPLOITATION (Turn 14+)
            
        # Select the best move from the policy
        final_move_tensor = torch.argmax(log_probs_all.squeeze(0))
            
        # Log probability is calculated from the model's distribution
        log_prob_of_chosen_action = dist.log_prob(final_move_tensor).unsqueeze(0)


    # 3. Log the chosen action's probability for the training step
    probabilities.append(log_prob_of_chosen_action.squeeze(0))

    # 4. Convert the chosen index to the environment command
    turn += 1
    move_index = final_move_tensor.item()
    move='UP'
    match move_index:
        case 0: move='UP'
        case 1: move='DOWN'
        case 2: move='LEFT'
        case 3: move='RIGHT'
        case 4: move="UP:BOOST"
        case 5: move="DOWN:BOOST"
        case 6: move="LEFT:BOOST"
        case 7: move="RIGHT:BOOST"
